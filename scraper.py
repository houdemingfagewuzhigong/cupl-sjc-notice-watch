#!/usr/bin/env python3
"""Fetch CUPL Guangming School of Journalism and Communication notices."""
...
BASE_URL = "https://sjc.cupl.edu.cn"
USER_AGENT = "Mozilla/5.0 (compatible; cupl-sjc-notice-watch/1.0; +https://github.com/houdemingfagewuzhigong)"
DATA_DIR = Path("data")

SECTIONS = [
    {"name": "通知公告", "path": "xydt/tzgg1.htm"},
    {"name": "学院新闻", "path": "xydt/xyxw.htm"},
]
...
def title_from_detail(url: str, fallback: str) -> str:
    ...
    title = re.sub(r"\s*[-_—|]\s*中国政法大学光明新闻传播学院\s*$", "", title).strip()
...
def parse_list(html_text: str, section: str, source_url: str) -> list[Notice]:
    seen_at = now_iso()
    items: list[Notice] = []
    item_pattern = re.compile(r"<p[^>]*>(.*?)</p>", re.S | re.I)
    for block_html in item_pattern.findall(html_text):
        ...
        date_match = re.search(r"<span[^>]*>(.*?)</span>", block_html, re.S | re.I)
...
def save(notices: list[Notice]) -> None:
    ...
    meta = {
        "site": "中国政法大学光明新闻传播学院",
        "base_url": BASE_URL,
        "notice_url": BASE_URL + "/xydt/tzgg1.htm",
        "updated_at": seen_at,
